...
export async function POST(
  ...
  try {
    ...
    // Only export subject calendars (exclude base)
    const subjects = ['ela', 'math', 'science', 'social_studies'];
    const subjectFilenames: Record<string, string> = {
      ela: 'ELA',
      math: 'Math',
      science: 'Science',
      social_studies: 'Social-Studies',
    ...
    for (const subject of subjects) {
      const calendar = calendars.find((c) => c.subject === subject);
      ...
      const html = generateCalendarHTML(guide, subject, events, components);
      ...
      // Add PDF to ZIP
      zip.file(
        `${guide.school_name}-${subjectFilenames[subject]}-Pacing-Guide.pdf`,
        pdfBuffer
      );
    }
    ...
    return new NextResponse(new Uint8Array(zipBuffer), {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${guide.school_name}-Pacing-Guides.zip"`,
      },
    });
  } catch (error) {
